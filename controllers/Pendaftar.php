		...
		public function cetak_formulir_adm($id)
			{
				...
				$pdf = new FPDF('P','mm',array(215,330));
						 ...
						 $pdf->SetY(265);
						 $pdf->Cell(7,7,'');
						 $pdf->Cell(65,7,'Bersedia mengikuti Tata Tertib / Aturan yang berlaku di TK IZZATUL ISLAM KARAWANG.');
	...
	public function cetak_formulir()
      {
        ...
        $pdf = new FPDF('P','mm',array(215,330));
             // membuat halaman baru
             $pdf->AddPage();
             $pdf->Image('./assets/backend/assets/images/izzatulkop.jpg',0,0,-160);
             // setting jenis font yang akan digunakan
             $pdf->Cell(50,45,'',0,1);
             $pdf->SetFont('Times','B',14);
             // mencetak string
             $pdf->Cell(200,6,'FORMULIR PENDAFTARAN',0,1,'C');
						 $pdf->Cell(200,6,'PENERIMAAN PESERTA DIDIK BARU (PPDB)',0,1,'C');
						 $pdf->Cell(200,6,'TAHUN PELAJARAN '.date('Y').'/'.(date('Y')+3),0,1,'C');
             $pdf->SetFont('Times','B',12);
             // $pdf->Cell(190,6,'Tanggal Cetak : '.date('d-m-Y H:i'),0,1,'C');
